mapping {
    ...
    section {
        when eventType().equalTo('imageevents') apply {
            map eventParameters().value('imageactions') onto 'imageactions'
            exit()
        }
        when eventType().equalTo('buttonevents') apply {
            map eventParameters().value('buttonactions') onto 'buttonactions'
            exit()
        }
    }
